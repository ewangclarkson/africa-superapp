  ...
  onTrack() {
    ...
    this.packageService.getPackage(this.mobileSearchValue).subscribe(
      (response: PackageResponse) => {
        if (response.activeDeliveryId.length > 0) {
          this.deliveryService.getDelivery(response.activeDeliveryId)
            .subscribe(
              (resp: DeliveryResponse) => {
                this.packageResponse = response;
                this.deliveryResponse = resp;
                this.setMarkerPosition(response.fromLocation.lat, response.fromLocation.lng,"http://maps.google.com/mapfiles/ms/icons/green-dot.png");
                this.setMarkerPosition(response.toLocation.lat, response.toLocation.lng,"http://maps.google.com/mapfiles/ms/icons/blue-dot.png");
                ...
                this.webSocketService.getMessage(Events.DELIVERY_UPDATED)
                  .subscribe((delivery: DeliveryResponse) => {
                    ...
                    if (delivery.deliveryId == this.deliveryResponse!.deliveryId) {
