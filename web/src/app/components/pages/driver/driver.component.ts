...
export class DriverComponent implements OnInit {
  ...
  generatePolyline(): google.maps.PolylineOptions {
    return {
      path: [this.packageResponse!.fromLocation, this.packageResponse!.toLocation],
      strokeColor: "#179138",
      strokeOpacity: 1.0,
      strokeWeight: 3
    };
  ...
  onSubmit() {
    ...
    this.deliveryService.getDelivery(this.mobileSearchValue).subscribe(
      (response: DeliveryResponse) => {
        if (response.packageId.length > 0) {
          this.packageService.getPackage(response.packageId)
            .subscribe(
              (resp: PackageResponse) => {
                this.packageResponse = resp;
                this.deliveryResponse = response;
                this.defineMapMarkers(resp.fromLocation.lat, resp.fromLocation.lng, "http://maps.google.com/mapfiles/ms/icons/green-dot.png");
                this.defineMapMarkers(resp.toLocation.lat, resp.toLocation.lng, "http://maps.google.com/mapfiles/ms/icons/blue-dot.png");
  ...
  setCurrentMapInformation() {
    navigator.geolocation.getCurrentPosition(position => {
      ...
      this.webSocketService.sendMessage(Events.LOCATION_CHANGE, {
        delivery_id: this.deliveryResponse!.deliveryId,
      ...
      this.markerPositions.push({
        ...
        iconUrl: "http://maps.gstatic.com/mapfiles/ms2/micons/bus.png"
      });
      this.showMap = true;
    });
  }
